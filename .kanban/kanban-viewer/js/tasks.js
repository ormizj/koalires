...
export function createTaskCard(task, status, progressEntry) {
            ...
            return `
                <div class="task-steps">
                    ...
                    <div class="task-steps-hidden" style="display: none;">
                        ${steps.slice(VISIBLE_FIRST, -SHOW_LAST).map((s) => `<div class="task-step">${escapeHtml(s)}</div>`).join('')}
                    </div>
  ...
  `
    ...
    const finalTokens = tokensUsed.length > 0 ? tokensUsed[tokensUsed.length - 1] : 0;
    const tokenLimit = 200000;
    const tokenPercent = finalTokens > 0 ? ((finalTokens / tokenLimit) * 100).toFixed(1) : 0;
    const tokenWarningClass = tokenPercent > 75 ? (tokenPercent > 90 ? 'danger' : 'warning') : '';
