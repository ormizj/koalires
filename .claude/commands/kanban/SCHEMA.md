...
type TaskCategory = "data" | "api" | "ui" | "integration" | "config" | "testing";
```
...
interface ProgressEntry {
  ...
  committed?: boolean;       // OPTIONAL - Default: false. True after git commit
...
| `committed` | boolean | No | Default `false`, set `true` after git commit |
...
The UI/viewer derives task status from multiple sources:

| `passes` | Progress Entry | `status` | `committed` | **Derived Status** |
|----------|----------------|----------|-------------|-------------------|
| `false` | None | - | - | **pending** |
...
| `true` | Exists | any | `false` | **code-review** |
| `true` | Exists | any | `true` | **completed** |
| `true` | None | - | - | **completed** |

**Note**: The `error` status in progress.json maps to **in-progress** in the viewer since verification hasn't passed. Only `blocked` has its own column.
