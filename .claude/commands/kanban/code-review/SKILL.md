...
This skill handles the code review and git commit phase of the kanban workflow. It processes tasks with `passes: true` and `committed: false` (code-review status), allowing the user to review changes and create individual commits for each approved task.

## Prerequisites

- `.kanban/kanban-board.json` must exist
- `.kanban/kanban-progress.json` must exist
- At least one task must be in code-review status (`passes: true`, `committed: false`)
...
3. Update progress.json: Set `committed: true` for this task
...
2. Update progress.json: Keep entry but ensure `committed: false`
...
On approve, set `committed: true`:

```json
{
  "task-name": {
    "log": "...",
    "committed": true,  // Updated
    "affectedFiles": [...],
    "agents": [...]
  }
}
