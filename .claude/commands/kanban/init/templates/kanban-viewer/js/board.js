...
export function renderBoard(boardData, progressData) {
  ...
  const minTokens =
    taskTokenValues.length > 0 ? Math.min(...taskTokenValues) : 0;
  const maxTokens =
    taskTokenValues.length > 0 ? Math.max(...taskTokenValues) : 0;
  const avgTokens =
    taskTokenValues.length > 0
      ? Math.round(totalTokens / taskTokenValues.length)
      : 0;
  ...
  document.getElementById('tasks-progress').innerHTML =
    inProgress.length > 0
      ? inProgress
          .map((t) => createTaskCard(t, 'in-progress', progressObj[t.name]))
          .join('')
  ...
  document.getElementById('tasks-review').innerHTML =
    codeReview.length > 0
      ? codeReview
          .map((t) => createTaskCard(t, 'code-review', progressObj[t.name]))
          .join('')
